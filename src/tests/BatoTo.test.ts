...
describe('BatoTo Tests', () => {
    ...
    const source: Source = new BatoTo(cheerio)
    ...
    it('Testing view more', async () => {
        const data = await wrapper.getViewMoreItems(source, '0', {page: null}) 
        expect(data, 'No server response').to.exist
        expect(data, 'Empty server response').to.not.be.empty
    })
    ...
    it('Testing Notifications', async () => {
        const updates = await wrapper.filterUpdatedManga(source, new Date('2021-1-27'), [mangaId])
